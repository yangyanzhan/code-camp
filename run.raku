...
my $contributor-disclaimer's = "Other Raku Contributors'";
...
sub my-build() {
  ...
  # roots of solutions
  my @roots = [["leetcode", "leetcode/c++", $cpp-extension], ["codesignal", "codesignal/c++", $cpp-extension], ["codewars", "codewars/c++", $cpp-extension], ["hackerrank", "hackerrank/c++", $cpp-extension], ["interviewbit", "interviewbit/c++", $cpp-extension]];
  for @roots -> @root {
      my $name = @root[0];
      my $path = @root[1];
      my $extension = @root[2];
...
sub gen-cpp($file, $filename, $judge-name, $judge-path) {
    ...
    if $contributor-disclaimer {
      $disclaimer's = $contributor-disclaimer's;
    }
    my @solution-lines = ["<head><title>{$title}</title></head>", "<body>", "<link rel=\"stylesheet\" href=\"/third-party/highlight/default.css\">", "<script src=\"/third-party/highlight/highlight.js\"></script>", "<script>hljs.initHighlightingOnLoad();</script>", "<h2>{$disclaimer's} for \"{$title}\"</h2>", "<ul>", @parts1.map( { "<li>{$_}</li>" } ).join("\n"), "</ul>", "<pre class=\"cpp-hidden\"><code class=\"c++\">", @parts2.join("\n"), "</code></pre>",
