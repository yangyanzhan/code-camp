...
// Blog URL for this problem: https://yangyanzhan.github.io/codesignal/alphanumeric-less.html .
...
bool alphanumericLess(std::string s1, std::string s2) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (is_number(item1) && is_number(item2)) {
            string tmp1;
            regex_replace(back_inserter(tmp1), item1.begin(), item1.end(), regex("^0+"), "");
            if (tmp1.size() == 0) {
                tmp1 = "0";
            }
            string tmp2;
            regex_replace(back_inserter(tmp2), item2.begin(), item2.end(), regex("^0+"), "");
            if (tmp2.size() == 0) {
                tmp2 = "0";
            }
